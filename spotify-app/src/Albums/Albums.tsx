...
function Albums() {
  ...
  const { data, isLoading, isError } = useQuery("searchAlbums", async () => {
    try {
      const response: any = await axios.get(
  ...
  return (
    <>
      ...
      <h1 style={{ paddingTop: "30px", paddingLeft: "80px" }}>
        {artist?.name}
      </h1>
            ...
            <div className="albums-card" key={album.id}>
              ...
              <h6 className="header-first">
                {album.artists.map((art: any, index: number) =>
                  album.artists.length == 0 || album.artists.length - 1 == index
                    ? art.name
                    : art.name + " , "
                )}{" "}
              </h6>
              <h6 className="header-second">{album.release_date}</h6>
              <h6 className="header-third">
                {album.total_tracks}{" "}
